...
2. **Aggregate into benchmark** — run the aggregation script from the skill-creator directory:
   ...
   ```
   This produces `benchmark.json` and `benchmark.md` with pass_rate, time, and tokens for each configuration, with mean ± stddev and the delta. If generating benchmark.json mannually, see `references/schemas.md` for the exact schema the viewer expects.
